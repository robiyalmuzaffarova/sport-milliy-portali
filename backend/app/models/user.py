...
class VerificationStatus(str, enum.Enum):
    """Profile verification status (athlete/trainer identity check)"""
...
class User(BaseModel):
    ...
    # Verification status (unverified/pending/verified/rejected). is_verified above
    # is kept in sync (True only when status == verified) for backward compatibility
    # with any existing code/frontend reading the boolean.
    ...
    # Legacy free-text achievements summary, kept for backward compatibility.
    # Real structured achievements now live in the `achievements` table
    # (see Achievement model / achievement_entries relationship below).
    ...
    # Structured achievements (achievements table). Named achievement_entries to
    # avoid clashing with the legacy `achievements` Text column above.
    achievement_entries = relationship("Achievement", back_populates="user", cascade="all, delete-orphan")
    gallery_photos = relationship("GalleryPhoto", back_populates="user", cascade="all, delete-orphan")
